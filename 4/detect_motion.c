...
int main(int argc, char* argv[]){
    ...
    // input file reader
    {
        ...
        for (i = 0; i < frame; i++) {
            sprintf(name, "%s%02d.teo", argv[1], i);
            src_teofp = TeoOpenFile(name);
            src_img[i] = TeoAllocSimilarImage(src_teofp);
    ...
    // Sub image
    {
        ...
        // 初期化
        for (i = 0; i < frame-1; i++) sub_img[i] = TeoAllocSimilarImage(src_img[i]);
        for (i = 0; i < frame-1; i++) {
    ...
    // And sub_image
    {
        for(i = 1; i< frame - 1; i++) {
            ...
            sprintf(name, "%s%02d.teo", argv[3], i);
